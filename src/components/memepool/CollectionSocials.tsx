...
import EditSocialsModal from "../token/EditSocialsModal";

type CollectionSocialsParams = {
  token: Token;
};

export default function CollectionSocials({ token }: CollectionSocialsParams) {
  ...
  return (
    <div className="flex gap-x-1 items-center mr-1 tablet:mr-0">
      {currentUser?.id === token.userId && (
        <button onClick={onOpen}>
          <MdOutlineEdit className="w-6 h-6 tablet:w-7 tablet:h-7 text-white/80 hover:text-white hover:scale-[1.02] transition-all" />
        </button>
      )}
      <Link
        href={token.telegram || ""}
      ...
      >
        <FaTelegram className="w-5 h-5 tablet:w-6 tablet:h-6" />
      </Link>
      <Link
        href={token.website || ""}
        target="_blank"
      ...
      >
        <FaGlobe className="w-5 h-5 tablet:w-6 tablet:h-6" />
      </Link>
      <Link
        href={token.twitter || ""}
        target="_blank"
      ...
      >
        <FaXTwitter className="w-5 h-5 tablet:w-6 tablet:h-6" />
      </Link>
      <Dropdown>
        <DropdownTrigger className="outline-none">
          <button>
            <IoMdShareAlt className="w-8 h-8 tablet:w-10 tablet:h-10 text-white/80 hover:text-white hover:scale-[1.02] transition-all" />
          </button>
        </DropdownTrigger>
